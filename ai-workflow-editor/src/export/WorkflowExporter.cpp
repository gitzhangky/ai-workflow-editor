...
WorkflowExporter::Result WorkflowExporter::generatePythonLangChain(QJsonObject const &workflow)
{
    ...
    for (auto const &node : sorted) {
        if (node.typeKey != QStringLiteral("agent"))
            continue;

        QStringList missingFields;
        if (node.properties[QStringLiteral("modelName")].toString().trimmed().isEmpty())
            missingFields << QStringLiteral("model");
        if (node.properties[QStringLiteral("agentInstructions")].toString().trimmed().isEmpty())
            missingFields << QStringLiteral("instructions");

        if (!missingFields.isEmpty()) {
            return {false,
                    {},
                    QStringLiteral("Agent node \"%1\" is missing required %2.")
                        .arg(node.displayName, missingFields.join(QStringLiteral(" and ")))};
        }
    }

    QMap<QString, int> indexByVariableName;
    QMap<int, int> indexById;
    for (int i = 0; i < sorted.size(); ++i) {
        indexByVariableName[sorted[i].variableName] = i;
        indexById[sorted[i].id] = i;
    }

    QStringList chainParts;
    QMap<QString, QString> promptLlmChainByLlmVar;
    for (auto const &conn : connections) {
        if (!indexById.contains(conn.outNodeId) || !indexById.contains(conn.inNodeId))
            continue;

        auto const &upstream = sorted[indexById.value(conn.outNodeId)];
        auto const &downstream = sorted[indexById.value(conn.inNodeId)];
        if (upstream.typeKey == QStringLiteral("prompt") && downstream.typeKey == QStringLiteral("llm")) {
            const QString chainName = QStringLiteral("%1_chain").arg(upstream.variableName);
            chainParts << QStringLiteral("%1 = %2 | %3").arg(chainName, upstream.variableName, downstream.variableName);
            promptLlmChainByLlmVar[downstream.variableName] = chainName;
        }
    }
    ...
    QStringList body;
    bool addedChatOutputHelper = false;
    ...
    for (auto const &node : sorted) {
        ...
        if (type == QStringLiteral("chatOutput")) {
            ...
            if (!importedModules.contains(QStringLiteral("re"))) {
                imports << QStringLiteral("import re");
                importedModules.insert(QStringLiteral("re"));
            }
            if (!addedChatOutputHelper) {
                body << QStringLiteral("def _resolve_template_value(data, path):");
                body << QStringLiteral("    current = data");
                body << QStringLiteral("    for part in path.split(\".\"):");
                body << QStringLiteral("        if isinstance(current, dict):");
                body << QStringLiteral("            current = current.get(part)");
                body << QStringLiteral("        else:");
                body << QStringLiteral("            current = getattr(current, part, None)");
                body << QStringLiteral("        if current is None:");
                body << QStringLiteral("            return None");
                body << QStringLiteral("    return current");
                body << QStringLiteral("");
                body << QStringLiteral("def _render_message_template(template, data):");
                body << QStringLiteral("    def replace(match):");
                body << QStringLiteral("        key = match.group(1).strip()");
                body << QStringLiteral("        value = _resolve_template_value(data, key)");
                body << QStringLiteral("        if value is None and key in (\"result\", \"content\"):");
                body << QStringLiteral("            value = data");
                body << QStringLiteral("        return str(value) if value is not None else match.group(0)");
                body << QStringLiteral("    return re.sub(r\"\\{\\{\\s*([^{}]+)\\s*\\}\\}\", replace, template)");
                body << QStringLiteral("");
                addedChatOutputHelper = true;
            }
            body << QStringLiteral("# %1").arg(node.displayName);
            body << QStringLiteral("def %1(content):").arg(node.variableName);
            body << QStringLiteral("    message_template = %1").arg(
                multilineString(tmpl.isEmpty() ? QStringLiteral("{{result}}") : tmpl));
            body << QStringLiteral("    rendered_content = _render_message_template(message_template, content)");
            body << QStringLiteral("    return {\"role\": \"%1\", \"content\": rendered_content}")
        ...
        if (type == QStringLiteral("output")) {
            ...
            if (!node.upstreamVars.isEmpty()) {
                const QString upstreamVar = node.upstreamVars.first();
                if (promptLlmChainByLlmVar.contains(upstreamVar)) {
                    body << QStringLiteral("result = None  # produced by run_workflow()");
                } else {
                    body << QStringLiteral("result = %1").arg(upstreamVar);
                }
            } else {
    ...
    const bool hasPromptLlmChain = !chainParts.isEmpty();
    ...
    auto runnableExpressionFor = [&](QString const &variableName, QString const &inputName) -> QString {
        if (promptLlmChainByLlmVar.contains(variableName)) {
            return QStringLiteral("%1.invoke({\"input\": %2})")
                .arg(promptLlmChainByLlmVar.value(variableName), inputName);
        }

        if (indexByVariableName.contains(variableName)) {
            auto const &sourceNode = sorted[indexByVariableName.value(variableName)];
            if (sourceNode.typeKey == QStringLiteral("chatOutput")) {
                const QString contentExpression = sourceNode.upstreamVars.isEmpty()
                                                      ? inputName
                                                      : sourceNode.upstreamVars.first();
                const QString renderedContent = promptLlmChainByLlmVar.contains(contentExpression)
                                                    ? QStringLiteral("%1.invoke({\"input\": %2})")
                                                          .arg(promptLlmChainByLlmVar.value(contentExpression), inputName)
                                                    : contentExpression;
                return QStringLiteral("%1(%2)").arg(variableName, renderedContent);
            }
        }
    ...
    QString runExpression = QStringLiteral("None");
    for (auto const &node : sorted) {
        if (node.typeKey == QStringLiteral("output") && !node.upstreamVars.isEmpty()) {
            runExpression = runnableExpressionFor(node.upstreamVars.first(), QStringLiteral("input_text"));
            break;
        }
    }
    if (runExpression == QStringLiteral("None") && hasPromptLlmChain)
        runExpression = QStringLiteral("%1.invoke({\"input\": input_text})").arg(chainParts.first().section(QStringLiteral(" = "), 0, 0));

    // Main execution block
    body << QStringLiteral("");
    body << QStringLiteral("def run_workflow(input_text=\"Hello\"):");
    body << QStringLiteral("    result = %1").arg(runExpression);
    body << QStringLiteral("    return result");
    body << QStringLiteral("");
    body << QStringLiteral("if __name__ == \"__main__\":");
    body << QStringLiteral("    result = run_workflow()");
    body << QStringLiteral("    print(result)");
    body << QStringLiteral("");
