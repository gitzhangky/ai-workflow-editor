...
也就是说，它现在主要解决的是“把工作流结构设计清楚、用 Mock 预览数据流并保存下来”，而不是直接调用真实模型执行生产任务。
...
7. 底部问题面板  
   汇总当前工作流中所有 warning / error。点击某条问题会自动选中对应节点、居中画布，并在右侧 Inspector 高亮需要修改的字段。
...
3. 选中 `提示词`，填写“用户提示模板”
4. 选中 `大模型`，填写“模型名称”
5. 点击 `运行预览`，输入一段测试文本，看每个节点的输入 / 输出追踪
6. 点击 `保存`

做完这 6 步，你就已经完成了一个最小可保存、可本地预览的 AI 工作流草图。
...
6. 打开 `运行预览`，输入一段测试文本，确认链路追踪是否符合预期
7. 按 `Command+S` 或点击 `保存`
8. 关闭后重新打开刚才的 JSON
...
点击工具栏或菜单里的 `运行预览`，会在中央工作区打开一个新的 `运行预览` 页签。

你可以在里面输入一段测试文本，然后点击 `运行预览` 按钮。编辑器会按当前工作流结构生成一次确定性的本地 Mock 追踪：

- `开始` 节点会接收测试输入
- `提示词` 节点会把模板里的 `{{input}}` 替换成上一步输出
- `大模型`、`Agent`、`工具`、`HTTP 请求` 等节点会返回可读的 Mock 占位结果
- `输出` / `聊天输出` 节点会展示最终输出
...
这个功能的目标是让你在接入真实执行引擎之前，先检查“数据有没有按预期从一个节点流到下一个节点”。
...
当画布上的节点变多时，可以先框选或多选多个节点，再通过菜单 `视图 -> 整理` 调整布局：

- `左对齐` / `右对齐`：把多个节点的左边或右边排齐
- `顶部对齐` / `底部对齐`：把多个节点的上边或下边排齐
- `水平分布`：按当前左右边界，把 3 个及以上节点在水平方向均匀排开
- `垂直分布`：按当前上下边界，把 3 个及以上节点在垂直方向均匀排开

这些整理操作会进入 Undo / Redo。手动拖动节点位置后，释放鼠标也会把位置写回工作流状态；保存、撤销、重做和小地图都会使用同一份位置数据。
...
2. 看底部 `问题` 面板里还有哪些问题
3. 点击某一行问题
4. 编辑器会自动跳到对应节点，Inspector 会显示并高亮对应字段
5. 修复后，对应问题会从列表里消失

如果暂时不需要查看问题列表，可以通过 `视图 -> 显示问题面板` 隐藏或重新显示。
...
点击 `运行预览` 会在中央工作区打开预览页签。这个页签和帮助文档一样会复用已有页面；重复点击不会创建多个相同的运行预览。

常见用法：

1. 在画布上搭好一条主流程
2. 修复底部 `问题` 面板里的关键 warning / error
3. 打开 `运行预览`
4. 输入样本文本
5. 点击预览按钮
6. 查看追踪表里每个节点的输入和输出

如果你修改了画布、Inspector 字段或连线，运行预览会使用最新的工作流内容。它适合在导出前快速确认模板变量、提示词拼接和输出格式是否大致符合预期。
...
以后开发任何面向用户的新功能必须同步更新帮助文档，包括：

- 界面内置的 `帮助 > 用户指南`
- 这份 `docs/user-guide.md`
- `docs/README.md` 里的入口说明
- 对应的 app / theme 测试，确保关键菜单、快捷键、节点类型、导出格式和重要交互不会漏写

如果新增节点、菜单、快捷键、导出格式、画布交互或 Inspector 字段，请把“用户应该如何使用、出错时怎么看、和现有功能有什么关系”一起写进帮助文档。这样帮助页会跟产品一起长大，而不是变成过期说明。
